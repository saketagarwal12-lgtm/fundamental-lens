...
// Shows the grade label (no numeric %).
export const FactorHeatmap: React.FC<{ factors: ScorecardFactor[] }> = ({ factors }) => (
  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
    {factors.map(f => {
      const grade = gradeForPct(f.pct);
      const c = gradeBarColor(grade);
      return (
        <div
          key={f.name}
          title={`${f.name}: ${grade}`}
        ...
        >
          ...
          <span className="text-[12px] font-semibold mt-1" style={{ color: c }}>{grade}</span>
        </div>
      );
    })}
  </div>
